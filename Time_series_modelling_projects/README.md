# About the project
The emphasis is on exploring the best model to use for forecasting the Australia production data using classic models as bench marks.
In this process, I used ETS, Naive, ARIMA, Exponential smoothing and the seasonal naive 

## Time plot
<img width="981" height="630" alt="1" src="https://github.com/user-attachments/assets/ae658e23-ccf6-4f8e-88d8-fa61e0336520" />

## Residual analysis
1. Naive
<img width="822" height="620" alt="2" src="https://github.com/user-attachments/assets/593506c4-b35d-416c-82f0-4f2a3180a8db" />
2. ARIMA
<img width="753" height="630" alt="3" src="https://github.com/user-attachments/assets/a2a74b0e-2362-4c75-848b-86c3b129bc16" />
3. ETS
<img width="753" height="630" alt="4" src="https://github.com/user-attachments/assets/11df8041-a1de-43fe-b1d3-9a3f053ba736" />

## Combined forecasts
<img width="753" height="630" alt="5" src="https://github.com/user-attachments/assets/7b8e7d6b-57f1-4b6d-8b65-a568a4cbf630" />

## Forecast evaluation
<img width="2225" height="334" alt="accuracy_table" src="https://github.com/user-attachments/assets/e23eed77-b24c-4890-85de-6e03b1f26a50" />





